...
description: Universal AI-agent workflow for building or repairing customizable WordPress/WooCommerce SEO sites using Hello Elementor, Elementor HTML blocks, Code Snippets PHP/CSS/JS, Rank Math, varied page designs, full-store QA, intelligent CSV product/media integrity, reference-site layout capture/adaptation, and autonomous or ask-user execution modes. Use with Codex, Claude, Trae, Cursor, OpenHands, Aider, or other AI coding agents when the user wants requirements intake, WordPress operation, old-site rebuilds, reference-site/clone-style builds, homepage/style previews, pages/products/categories/policies/blogs, product CSV originality rewriting that intelligently rewrites product names, short descriptions, long body descriptions, image text, and Rank Math metadata rather than merely translating or preserving source copy, non-repetitive layouts, snippet implementation, interaction testing, launch gating, or a reusable automated WordPress site-building workflow.
---
...
8. **Build and verify the global shell BEFORE any page HTML**. Read `references/global-shell-architecture.md` and `references/site-baseline-and-menus.md`. Create the global header and footer/menu via persistent Code Snippets (`wp_body_open` and `wp_footer`), put design tokens/header/footer/shared CSS in Appearance -> Customize -> Additional CSS, add global JS through Code Snippets, create/assign header and footer menus, and verify on front-end desktop/mobile. **Do NOT generate, paste, import, or update production page HTML until this gate passes.**
...
13. **Generate page HTML (one page at a time)** for Home, Blog, Contact, policies, FAQ, and About using one Elementor HTML block per page. Each page contains ONLY page-specific HTML, scoped CSS (`.page-name .class`), and scoped JS (IIFE). No header, no footer, no menu, no global CSS/JS 鈥?the global shell handles all shared elements. Each page loads only its own CSS/JS. **DEAD RULE: Homepage and Blog MUST use dynamic data containers (`data-site-render="home-products"`, etc.) filled by Code Snippets PHP 鈥?never hardcode product/article data. See "CRITICAL: Additional Dead Rules" Dead Rule 5.** **Import one page at a time: Canvas 鈫?verify 鈫?clear existing layout/default widgets 鈫?add one HTML widget 鈫?paste HTML 鈫?Update 鈫?verify front-end 鈫?next page.**
