...
description: Universal AI-agent workflow for building or repairing customizable WordPress/WooCommerce SEO sites using Hello Elementor, Elementor HTML blocks, Code Snippets PHP/CSS/JS, Rank Math, frontend UI aesthetic quality gates, varied page designs, full-store QA, intelligent CSV product/media integrity, reference-site adaptation, and autonomous or ask-user execution modes. Use with Codex, Claude, Trae, Cursor, OpenHands, Aider, and other AI coding agents for requirements intake, WordPress operation, old-site rebuilds, reference-site builds, homepage/style previews, polished global header/footer/menu systems, pages/products/categories/policies/blogs, product CSV originality rewriting for names/descriptions/image text/Rank Math fields, non-repetitive layouts, snippets, interaction testing, launch gating, or reusable automated WordPress site-building workflow.
---
...
## CRITICAL: Frontend UI Aesthetic Gate

Generated HTML must look like a deliberate ecommerce UI, not a generic AI landing page. Before homepage preview, global shell work, or production page HTML, read `references/frontend-ui-aesthetic-system.md`, `references/design-variation.md`, and `references/global-design-preferences.md`.

- Define a target-market-aware design system first: palette, typography, spacing, radii, shadows, button styles, form styles, product-card rules, icon usage, mobile drawer behavior, header/footer layout, and responsive breakpoints.
- Use open-source UI systems as pattern inspiration only: shadcn/ui, Radix UI, Headless UI, Tailwind UI, Lucide-style icons, Shopify ecommerce patterns, and Linear/Stripe-level spacing discipline. Do not copy proprietary templates, code, assets, logos, screenshots, or copyrighted designs.
- Put global tokens, shared components, header/footer styles, and responsive rules in Appearance -> Customize -> Additional CSS before page HTML.
- Build and verify polished global header/footer/menu before page HTML. Header and footer must have stable spacing, logo sizing, hover/focus states, mobile drawer behavior, contact/policy/payment/support/compliance links, and target-market trust cues.
- Page HTML must inherit the design system and vary sections by purpose. Do not ship hero-only pages, repetitive three-card filler, one-note color palettes, default Elementor styling, generic gradients, decorative blobs, fake reviews, fake awards, or placeholder content.
- Visual QA is mandatory: desktop/mobile screenshots, 360/390/430px overflow checks, no clipped text/buttons, no blocked taps, and ledger fields for `frontend_ui_system_defined`, `tokens_defined`, `header_aesthetic_pass`, `footer_aesthetic_pass`, and `page_aesthetic_pass`.
...
- Product CSV rewriting is not plain translation and not light paraphrasing. For customer-facing fields, intelligently rewrite product `Name`, `Short description`, `Description`/long body content, editable image text, and Rank Math product SEO from the product knowledge ledger, target market, buyer intent, category strategy, and compliance limits.
- Product CSV rewriting must follow the same site context as the build: target country, small-language/local-language choice, single-language vs multilingual architecture, approved or autonomous design style, brand replacement rules, payment method, shipping origin/timing, support channels, currency conversion, and regulated-product constraints.
- Brand replacement must never rewrite image URL strings or remote media paths, even when the old brand appears inside the URL. Rewriting image URLs can break product imports and galleries. Brand replacement is allowed in customer-facing text, editable ALT/title/caption fields, and SEO metadata only when the field is explicitly editable.
- Avoid catalog-wide sameness. Product names, short descriptions, long descriptions, SEO titles, and meta descriptions must vary by product type, category, attribute, use case, differentiator, and buyer question; do not keep the original CSV wording pattern across the catalog.
- Rewrite long product body content as a useful purchase page: reorganize thin or duplicate source text into structured sections such as overview, key specs, use cases, compatibility/fit, package contents, care/use notes, shipping/returns, and FAQ when the facts support them. Preserve inline/body images and valid HTML.
- Never invent specs, certifications, reviews, guarantees, shipping promises, medical/safety claims, or stock facts to make the rewrite look richer. Mark missing facts and use only truthful, supportable copy.
- If the user authorizes autonomous mode, the agent may choose a conservative single-language or multilingual strategy from the target market and product facts, but must record the decision, language codes/locales, currency, payment/shipping assumptions, and how those choices shaped product names, descriptions, metadata, and homepage/category merchandising.
...
- After product import, convert remote/hotlinked product images into local WordPress Media Library attachments with a suitable image/import optimization plugin or controlled media sideload workflow, then convert/serve them as WebP. Do not leave product, gallery, category, article, hero, logo, or other site images hotlinked unless the user explicitly approved a CDN/external-media architecture.
- Any related images or other image resources selected for pages, posts, products, categories, OpenGraph, logo, or design assets must be uploaded/localized and converted to WebP before use, or served through a verified WebP-capable media pipeline with fallback.
...
- **Clean canvas before HTML**: For each custom Elementor page being generated or rebuilt, clear the existing/default Elementor layout before adding the new HTML widget. Remove default sections, placeholder containers, demo widgets, old generated HTML widgets, empty spacers, and stale page-builder blocks so the new page does not stack on top of prior content.
- **Preserve human content**: Do not clear an existing human-built page unless the user approved overwrite/rebuild, the page is a new draft, or the content is verified stale generated output from the current task. Record what was cleared in the page ledger.
- **Hard page-HTML gate**: Before generating, pasting, importing, or updating any production Elementor page HTML code, the global header, footer menu/footer, Additional CSS, and global JS must already be created, active, and verified on the front end.
- **Additional CSS is the global CSS home**: Design tokens, header/footer styles, shared button/form/card styles, and responsive breakpoints go in Appearance -> Customize -> Additional CSS, not inside page HTML widgets.
- **Code Snippets owns global shell behavior**: Header/footer/menu output and global JS must be implemented through persistent Code Snippets using `wp_body_open` and `wp_footer`, with `wp_nav_menu()` for menus. Do not hardcode menus into page HTML.
- **Ledger evidence required**: Record snippet names/IDs, hook names, menu source, Additional CSS status, global JS status, tested URLs, desktop/mobile menu result, footer-link result, and proof that page HTML has no duplicated header/footer/global CSS/JS.
- **Allowed preview exception**: A local/homepage style preview may show a proposed header/footer direction for approval, but production page HTML generation/import still cannot begin until the real global shell is implemented and verified.
...
12. Read `references/frontend-ui-aesthetic-system.md` and `references/design-variation.md` before generating previews, global shell CSS, page HTML/CSS, or article layouts so every site has a polished open-source-inspired UI system and a distinct, content-rich, non-AI-looking design direction.
...
- **Before adding the HTML widget, clear the page canvas of any default layout, placeholder section, old generated widget, duplicate HTML block, empty container, or stale Elementor content.** Only then add the intended single HTML widget for that page. Do not clear protected human content without approval.
...
- **Follow the step-by-step import order: global header/footer/menu first, then global CSS in Appearance -> Customize -> Additional CSS, then global JS in Code Snippets, then dynamic renderers, then page HTML one page at a time.** Never generate or import production page HTML before the global shell is active and verified. **This is a DEAD RULE 鈥?see "CRITICAL: Additional Dead Rules" above.**
...
- Use a resumable page ledger: each page has status `created`, `canvas_set`, `canvas_cleared`, `html_inserted`, `updated`, `verified`, with `cleared_items` recorded. If one step fails, resume from the failed step instead of deleting and starting over.
...
7. **Generate homepage style preview** using real site inputs, product/category signals from the product knowledge ledger, proposed navigation, and scoped HTML/CSS. Apply global design preferences and the frontend UI aesthetic system for the target market. Pause for user approval. Read `references/style-preview-gate.md` and `references/frontend-ui-aesthetic-system.md`.
8. **Build and verify the frontend UI aesthetic system and global shell BEFORE any page HTML**. Read `references/frontend-ui-aesthetic-system.md`, `references/global-shell-architecture.md`, and `references/site-baseline-and-menus.md`. Define design tokens and shared component rules, create the global header and footer/menu via persistent Code Snippets (`wp_body_open` and `wp_footer`), put design tokens/header/footer/shared CSS in Appearance -> Customize -> Additional CSS, add global JS through Code Snippets, create/assign header and footer menus, and verify front-end desktop/mobile aesthetics and interactions. **Do NOT generate, paste, import, or update production page HTML until this gate passes.**
9. **Product CSV rewriting/import prep**: If the user provides a WooCommerce product export CSV, intelligently rewrite product names/titles, short descriptions, long body descriptions, editable image text, and Rank Math product SEO fields for originality and buyer usefulness. This is not translation-only and not preserving the original CSV copy pattern. Align the rewrite with target language/locale, single-language or multilingual strategy, market design/content tone, brand replacement rules, payment/shipping facts, currency conversion, and compliance limits. Preserve import-critical identifiers, including image URLs and remote media paths, and produce a re-import-ready CSV plus change report. The schema inspection and product knowledge ledger must already be complete before homepage preview/content generation. Read `references/product-csv-originality-seo.md` and `references/rank-math-seo-guide.md`.
10. **Create product categories, attributes, products, variations**: SKU, slugs, prices, stock, images, galleries, and catalog visibility. Preserve CSV image URL strings during import, then localize/sideload remote product images into the Media Library and convert/serve all images as WebP before using them on product pages, page modules, articles, OpenGraph, logo, or category surfaces.
11. **After shell verification, set Elementor Canvas only for custom pages**. Read `references/elementor-html-automation.md`. Use the URL map for all links. Verify no 404s and verify Canvas removes theme chrome. **Do NOT import page HTML until Canvas and the global shell are both verified.**
12. **Clear the Elementor canvas before adding HTML**. For each custom page, remove default layouts, placeholder sections, old generated HTML widgets, duplicate widgets, empty containers, stale Elementor blocks, and other leftover content before adding the new HTML widget. Preserve existing human-written content unless overwrite/rebuild is authorized. Record `"canvas_cleared": true` and what was removed in the page ledger.
13. **Generate page HTML (one page at a time)** for Home, Blog, Contact, policies, FAQ, and About using one Elementor HTML block per page. Each page contains ONLY page-specific HTML, scoped CSS (`.page-name .class`), and scoped JS (IIFE). No header, no footer, no menu, no global CSS/JS 鈥?the global shell handles all shared elements. Each page loads only its own CSS/JS and must inherit the approved frontend UI aesthetic system without default Elementor styling, repetitive filler cards, or one-note palettes. **DEAD RULE: Homepage and Blog MUST use dynamic data containers (`data-site-render="home-products"`, etc.) filled by Code Snippets PHP 鈥?never hardcode product/article data. See "CRITICAL: Additional Dead Rules" Dead Rule 5.** **Import one page at a time: Canvas 鈫?verify 鈫?clear existing layout/default widgets 鈫?add one HTML widget 鈫?paste HTML 鈫?Update 鈫?verify front-end 鈫?next page.**
...
- `references/frontend-ui-aesthetic-system.md`: open-source-inspired frontend UI aesthetic system, design tokens, header/footer quality gates, ecommerce component standards, and visual QA.
