...
When rewriting product CSV files, do intelligent originality rewriting of product names, short descriptions, long body descriptions, editable image text, and Rank Math fields from verified facts; align the rewrite with target country/language, single-language or multilingual strategy, design/content style, brand replacement rules, payment/shipping facts, and currency; do not merely translate or keep the original CSV copy pattern.
...
- If a tool rewrites product CSV content, it must rewrite product names, short descriptions, long descriptions/body content, editable image text, and Rank Math fields from the product knowledge ledger plus the target-market/design/commerce context. Translation-only, synonym-only, mixed-language fields, currency mistakes, unsupported delivery/payment promises, or catalog-wide repeated formulas are blockers.
