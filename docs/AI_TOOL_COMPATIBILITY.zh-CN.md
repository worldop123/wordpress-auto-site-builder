...
改写产品 CSV 时，必须基于已核实的产品事实，结合目标国家/语言、单语言或多语言策略、设计/内容风格、品牌替换规则、支付/物流事实和币种，智能重写产品名、短描述、长正文、可编辑图片文字和 Rank Math 字段；不能只是翻译，也不能保留原 CSV 的标题/描述/正文套路。
```
...
- 如果工具改写产品 CSV，必须根据产品知识台账以及目标市场/设计/交易上下文重写产品名、短描述、长正文/正文内容、可编辑图片文字和 Rank Math 字段。只翻译、只换同义词、字段混用多语言、币种错误、写入无依据的配送/支付承诺、全目录套同一公式，都算 blocker。
